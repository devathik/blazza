...
import { ChevronRight, UserCheck, ShieldCheck, ArrowRight } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-8 space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full bg-primary-surface border border-primary/20 px-3.5 py-1 text-xs font-bold text-primary shadow-2xs">
              ...
              <span>{t("membership_hero_badge")}</span>
            </div>

            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight text-foreground leading-tight">
              {t("membership_hero_title")}
            </h1>

            <p className="text-sm sm:text-base text-muted-foreground leading-relaxed font-medium max-w-2xl">
              {t("membership_hero_subtitle")}
            </p>

            <div className="flex flex-col sm:flex-row items-center gap-4 pt-2">
              <Link
                href="/membership/apply"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 text-sm font-extrabold text-primary-foreground shadow-md hover:bg-primary-dark transition-all"
              >
              ...
              <Link
                href="/login"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-card border border-border px-6 py-3 text-sm font-bold text-foreground hover:bg-primary-surface transition-all shadow-2xs"
              >
