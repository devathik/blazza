...
import { PublicationCategory } from "../types/publication.types";

interface PublicationsFilterBarProps {
  activeCategory: PublicationCategory;
  onCategoryChange: (cat: PublicationCategory) => void;
  searchQuery: string;
...
export default function PublicationsFilterBar({
  ...
  const categories: { id: PublicationCategory; label: string }[] = [
    { id: "all", label: t("pub_filter_all") },
    { id: "journal", label: t("pub_filter_journal") },
    { id: "manual", label: t("pub_filter_manual") },
    { id: "policy", label: t("pub_filter_policy") },
    { id: "report", label: t("pub_filter_report") },
  ];
    ...
    <div className="space-y-4 mb-8">
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        ...
        <div className="relative min-w-[240px]">
          ...
          <input
            ...
            placeholder={t("pub_search_placeholder")}
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground px-1">
        <div className="flex items-center gap-1.5 font-medium">
          ...
          <span>
            {language === "bn"
              ? `প্রদর্শিত হচ্ছে: ${resultCount} টি প্রকাশনা`
              : `Showing ${resultCount} publications`}
          </span>
        </div>
      </div>
    </div>
